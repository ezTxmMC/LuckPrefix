...
public class NameColorSubCommand {
    ...
    public static boolean execute(Audience adventurePlayer, Group group, String[] args, ConfigService configService) {
        ...
        if (args.length == 5) {
            try {
                try {
                    ...
                    groupsConfig.setNameColor(group.getName(), color.name().toLowerCase());
                    groupsConfig.save();
                    LuckPrefix.getInstance().getGroupManager().reloadGroup(group.getName());
                    String nameColor = groupsConfig.getNameColor(group.getName());
